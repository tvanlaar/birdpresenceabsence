...
```{r}
auk_set_ebd_path("~/Desktop/SiskinPresenceAbsence", overwrite=TRUE)
```

```{r}
ebd<-read_tsv("data/ebdonly.txt")
smp<-read_tsv("data/sampling.txt")
```

```{r}
##set file names for filtered files
f_ebd<-"data/ebd_pinsis.txt"
f_sed<-"data/sed_pinsis.txt"
...
```{r}
##write to file
write_csv(zf_effort, "data/ebird_pinsis_zf.csv")
```
