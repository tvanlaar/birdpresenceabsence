...
```{r}
auk_set_ebd_path("~/Desktop/BirdPresenceAbsence/corvidCA", overwrite=TRUE)
```
...
```{r}
f_ebd <- "ebd_corvid.txt"
f_sed <- "sed_corvid.txt"
...
# zero-fill
```{r}
corvid_zf <- auk_zerofill(f_ebd, f_sed, collapse = TRUE)
count(corvid_zf, scientific_name, species_observed)
```
...
##save as file
```{r}
write_csv(corvidebird, "corvideffort.csv")
```

```{r}
corvid_freq <- corvidebird %>%
...
```{r}
save(corvid_comm, file = "corvid_comm_1970_2024.RData")
```

```{r}
graph=ggplot(corvid_comm) +
...
```{r}
ggsave(
  filename="corvidCA19702024.tiff",
  plot = graph,
  width = 250,
  height = 200,
  units = c("mm"),
